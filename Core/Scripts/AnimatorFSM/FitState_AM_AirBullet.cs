...
public class FitState_AM_AirBullet : BaseFSMState
{
		...
		public FitState_AM_AirBullet()
		{
		}

		public FitState_AM_AirBullet(float init, bool JuDccl, bool FaFl)
		{
				InitVel = init;
				JuDccel = JuDccl;
				FastFall = FaFl;
...
//				}
	...
	public void CheckAerial() {
		...
		case Cardinals.Left:
			if (controller.x_facing == 1) {
				controller.FitAnima.Play ("SAirBullet",0,0f);
				break;
			} else {
				controller.FitAnima.Play ("SAirBullet",0,0f);
				break;
		...
		case Cardinals.Down:
			controller.FitAnima.Play ("DAirBullet",0,0f);
			break;
	...
	{
		//Check for Fast Fall
		if (controller.Inputter.y <= -0.7f && controller.Inputter.FramesYNeutral <= 5 && controller.velocity.y <= 0 && controller.Animator.CanFastFall) {
			FastFall = true;
		}
